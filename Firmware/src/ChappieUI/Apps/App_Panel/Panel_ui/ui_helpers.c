...
void _Panel_ui_basic_set_property(lv_obj_t * target, int id, int val)
{
    ...
    if(id == _Panel_UI_BASIC_PROPERTY_WIDTH) lv_obj_set_width(target, val);
    if(id == _Panel_UI_BASIC_PROPERTY_HEIGHT) lv_obj_set_height(target, val);
}
...
void _Panel_ui_state_modify(lv_obj_t * target, int32_t state, int value)
{
    ...
    else if(value == _Panel_UI_MODIFY_STATE_ADD) lv_obj_add_state(target, state);
